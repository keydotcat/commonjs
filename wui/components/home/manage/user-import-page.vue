...
export default {
  ...
  computed: {
    allOk() {
      if (this.currentStatus !== STATUS_LOADED) {
        return false
      }
      if (this.importPass.length === 0) {
        return false
      }
      if (this.parentVault.tid.length === 0) {
        return false
      }
      if (this.parentVault.vid.length === 0) {
        return false
      }
      return true
    },
